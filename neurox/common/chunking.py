...
def run_chunked[InputsT, OutputsT](
    *,
    ...
    Equal-sized chunks run in at most two scans; singleton groups run directly.
    Only real positions enter each callback. Expanded inputs are gathered on
    demand, and no positional output enters a scan carry. Complete outputs,
    ...
    Args:
        ...
        output_template: Result dataclass structure used by scan, including
            nested dataclasses and optional fields. Scan requires registered
        ...
        body_fn: Receives a dataclass whose tensor fields have one leading
            ...
            Scanned outputs match `output_template`; remaining dimensions stay fixed.
...
def _run_uniform_group[InputsT, OutputsT](
    *,
    ...
    def scan_body(start: Tensor) -> OutputsT:
