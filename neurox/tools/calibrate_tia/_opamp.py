...
methodology as :mod:`neurox.tools.calibrate_solver.nested`:
...
CLI: ``python -m neurox.tools.calibrate_tia._opamp --help``
"""
...
from neurox.primitive.analog import VoltageReferenceConfig
from neurox.primitive.analog.tia import OpAmpTia, OpAmpTiaConfig, OpAmpTiaPolicy, TiaConfig
...
@dataclass(frozen=True)
class CalibrateTiaConfig(ConfigBase):
    """Top-level config for :mod:`neurox.tools.calibrate_tia._opamp`.

    ``tia_config`` is the TIA fragment under calibration and
    ``clamp_ref_config`` the boundary-clamp reference sourcing its injected
    ``v_ref`` (tap 0); both pull from any scheme's chip params via
    ``_neurox_use``.
    """
...
def sweep_n_newton(
    *,
    ...
    ``v_ref__V`` is the BL-clamp tap from the chip's ``clamp_ref_config``,
    injected per call into :meth:`OpAmpTia.snapshot` — built into a 0-d
    tensor here and broadcast onto the snap shape.
    """
    rows: list[CandidateRow] = []
    i_port_typ__uA = float(port_currents__uA.abs().max().item())
    v_ref_tensor = torch.tensor(v_ref__V, dtype=dtype, device=device)
    ...
    for n_newton in candidates:
        ...
        snap = tia.snapshot(v_ref__V=v_ref_tensor, shape=(port_currents__uA.shape[0],), multi_coords=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = load_tool_config(CalibrateTiaConfig, args.config)
    log.info("loaded config from %s", args.config)

    tia_config = cfg.tia_config
    ...
    # BL-clamp reference tap (tap 0 of the chip's boundary-clamp reference):
    # the TIA's injected v_ref, sourced from the reference fragment rather
    # than the TIA config.
    bl_v_ref__V = cfg.clamp_ref_config.v_refs__V[0]
    rows, scale = sweep_n_newton(
        tia_config=tia_config,
        v_ref__V=bl_v_ref__V,
    ...
    log.info("")
    log.info("TOML fragment for the chip's tia_config table:")
