...
class Reporter:
    ...
    def dynamic_entries(self, profiler: Profiler) -> tuple[DynamicEntry, ...]:
        ...
        """
        energy_records = profiler.records
        energies = self._scalars__fJ(energy_records)
        merged: dict[tuple[str, ...], float] = {}
        for energy_record, energy in zip(energy_records, energies, strict=True):
            path = self._path_of(energy_record)
    ...
    def by_name(self, profiler: Profiler) -> dict[str, float]:
        ...
        for energy_record, energy in zip(energy_records, self._scalars__fJ(energy_records), strict=True):
            name = ".".join(self._path_of(energy_record))
    ...
    @staticmethod
    def _scalars__fJ(energy_records: Sequence[EnergyRecord]) -> list[float]:
        """Total each record's energy tensor to a host float in one device sync."""
        if not energy_records:
            return []
        return torch.stack([energy_record.dynamic_energy__fJ.sum() for energy_record in energy_records]).cpu().tolist()

    def _path_of(self, energy_record: EnergyRecord) -> tuple[str, ...]:
        ...
        Raises:
            ValueError: The record names a module outside the bound model, the
                ...
                module.
        """
        name = energy_record.qualified_name
