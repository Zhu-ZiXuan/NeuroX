...
See also:
    docs/reference/architecture/unit/cim/direct.md
"""
...
class DirectCimUnitConfig(CimUnitConfig):
    """Configuration for :class:`DirectCimUnit`.
...
@dataclass(frozen=True)
class DirectCimUnitPolicy(CimUnitPolicy):
    """Composite policy for :class:`DirectCimUnit`.
...
@CimUnit.register_key(DirectCimUnitConfig)
class DirectCimUnit(CimUnit):
    """CIM unit with no activation / weight slicing — transcode only.
