...
class RsCsaIadc(Iadc[RsCsaIadcConfig, RsCsaIadcPolicy]):
    ...
    _ladder_weights: Tensor  # Shape: [2**bits - 1]
    ...
    def __init__(
        ...
        *,
        ...
        self._energy_per_op_scale = tuple(t / self._window__ns[-1] for t in self._window__ns)
    ...
    def latency__ns(self, *, active_bits: int) -> float:
        ...
        return self._window__ns[active_bits - 1]
    ...
    def t_conversion__ns(self, active_bits: int) -> float:
        """Return the executed conversion window at `active_bits`.

        The window spans the compensation phase, the first `active_bits - 1` compare phases
    ...
    def _convert_impl(
        ...
        *,
        ...
        `bits` widens the effective bin to `2 ** (bits - active_bits)` current steps
        without moving the transfer: the code is taken at full resolution and its
        unresolved low bits are dropped, which is exactly what the first `active_bits`
        compare phases resolve.

        Energy and latency follow the EXECUTED phases: the per-phase compare term
        sums over the first `active_bits` references, the code-independent baseline is
        ...
        Returns:
            Unsigned integer code [int16] in `[0, 2 ** active_bits - 1]`, one code per
                `i_in__uA` element.
        ...
        # bounds the code by 2**bits - 1, so no clamp is needed.
        # Shape: [..., 1] * [2**bits - 1] -> [..., 2**bits - 1]
        ladder__uA = i_refs__uA * self._ladder_weights
        code = (i_comp__uA.unsqueeze(-1) >= ladder__uA).sum(dim=-1).to(torch.int16)

        # --- DATA-DEPENDENT energy: per-op baseline + phase conduction ---

        if self._is_dynamic_energy_profile_active():
            # The baseline holds for the executed window alone.
            e__fJ = torch.full_like(
                i_in__uA,
                config.energy_per_op__fJ * self._energy_per_op_scale[active_bits - 1],
            )
            # Only the first `active_bits` compare phases run, and the residue recursion
            ...
            for phase in range(1, active_bits + 1):
                # Phase p resolves bit `bits - p`, so it weighs the residue
                # against that bit's place value on the ONE reference.
                i_phase_ref__uA = (1 << (bits - phase)) * i_ref__uA
