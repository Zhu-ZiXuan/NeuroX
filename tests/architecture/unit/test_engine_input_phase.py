"""Tests for engine-level input-phase masking and accumulation."""
...
def _slice_config_kwargs(*, row_num: int, max_active_num: int) -> dict[str, Any]:
    return {
        "input_num": row_num,
        "output_num": 8,
        "cim_macro_config": _ideal_macro_config(max_active_num=max_active_num),
...
def _build_inter(
    *,
    ...
    config = InterArraySliceCimEngineConfig(
        **_slice_config_kwargs(row_num=row_num, max_active_num=max_active_num)
    )
...
def test_input_phase_mask_content() -> None:
    """input_num=8 and max_active_num=2 produce four disjoint masks."""
    engine = _build_direct(w_logical_shape=(4, 8), row_num=8, max_active_num=2)
    assert engine._input_phase_num == 4
    mask = engine._active_input_mask
...
def test_unroll_input_phase_layout() -> None:
    """P lands immediately left of the instance block and partitions inputs."""
    engine = _build_direct(w_logical_shape=(4, 8), row_num=8, max_active_num=2)
    p = engine._input_phase_num
    # Organized activation layout for the direct engine: [M, Tc, Tr, input_num].
    x = torch.arange(1, 17, dtype=torch.int64).reshape(2, 1, 1, 8)
    planes = engine._unroll_input_phase(x)
...
def test_unroll_input_phase_batch_axes_stay_left_of_p() -> None:
...
def test_input_phase_non_divisible_ceil_covers_all_inputs() -> None:
    ...
    engine = _build_direct(w_logical_shape=(4, 10), row_num=10, max_active_num=3)
    assert engine._input_phase_num == 4
    mask = engine._active_input_mask
...
def test_degenerate_input_phase_axis_size_one() -> None:
    ...
    engine = _build_direct(w_logical_shape=(4, 8), row_num=8, max_active_num=8)
    assert engine._input_phase_num == 1
...
def test_input_phase_count_skips_padding_only_blocks(build: Callable[..., CimEngine]) -> None:
    ...
    engine = build(w_logical_shape=(n, k), row_num=8, max_active_num=2)
    assert engine._input_phase_num == 2
    assert engine._active_input_mask.shape == (2, 8)
...
def test_engine_matmul_parity_non_divisible(build: Callable[..., CimEngine]) -> None:
    ...
    engine = build(w_logical_shape=(n, k), row_num=10, max_active_num=3)
    assert engine._input_phase_num == 4
    weight = _randint_in_range(engine.w_value_range, (n, k))
    activation = _randint_in_range(engine.x_value_range, (m, k))
    _assert_engine_matches_torch(engine, weight, activation)
...
def test_direct_engine_weight_batch_parity(activation_batch: tuple[int, ...]) -> None:
    """Weight batches preserve the input-phase reduction axis."""
    torch.manual_seed(11)
    b, n, k, m = 2, 5, 10, 3
    engine = _build_direct(w_logical_shape=(b, n, k), row_num=8, max_active_num=2)
