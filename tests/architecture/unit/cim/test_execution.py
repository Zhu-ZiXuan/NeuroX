...
def test_single_signed_slice_uses_local_polarity_recovery(modeled: bool, device: torch.device) -> None:
    ...
    if modeled:
        ...
        torch.testing.assert_close(item.dynamic_energy__fJ, torch.full((2,), 2.0, device="cpu"), check_dtype=False)
...
def test_adjacent_polarity_mapping_bills_macro_capacity_and_local_pair_throughput(
    ...
    torch.testing.assert_close(
        difference.dynamic_energy__fJ, torch.full((2, 3), 432.0, device="cpu"), check_dtype=False
    )
    torch.testing.assert_close(local.dynamic_energy__fJ, torch.full((2, 3), 486.0, device="cpu"), check_dtype=False)
...
def test_tile_recovery_applies_register_width_and_gates_padding(device: torch.device) -> None:
    ...
    torch.testing.assert_close(item.dynamic_energy__fJ, torch.full((2, 5), 54.0, device="cpu"), check_dtype=False)
...
def test_global_circuits_bill_each_tile_update_and_one_weight_recovery(device: torch.device) -> None:
    ...
    torch.testing.assert_close(
        weight_item.dynamic_energy__fJ, torch.full((2, 5), 12.0, device="cpu"), check_dtype=False
    )
    torch.testing.assert_close(tile_item.dynamic_energy__fJ, torch.full((2, 5), 36.0, device="cpu"), check_dtype=False)
...
def test_local_recovery_bills_valid_updates_and_allocates_one_circuit_per_lane(device: torch.device) -> None:
    ...
    torch.testing.assert_close(item.dynamic_energy__fJ, torch.full((2, 3), 486.0, device="cpu"), check_dtype=False)
