...
corners. Three regimes are pinned:
  ...
  * **vs dense LU**, fp64, across block dimension and `block_num`. Sets the
    absolute accuracy bar (rtol 1e-10).
...
Also covers `block_num = 1` (degenerate), `block_num = 2` (one PCR step), and
the M-matrix structure that mirrors the wire-Newton block-2×2 used in
production.
"""
...
def test_pcr_matches_dense_lu(block_size: int, block_num: int, device: torch.device) -> None:
    ...
    assert x_pcr.shape == (block_num, block_size)
...
def test_pcr_matches_thomas_fp64(block_num: int, device: torch.device) -> None:
    ...
    assert rel_err < 1e-10, f"N={block_num}: PCR vs Thomas rel err {rel_err.item():.2e}"


@pytest.mark.parametrize("block_num", [3, 8, 17, 64, 128, 256])
def test_pcr_matches_thomas_fp32(block_num: int, device: torch.device) -> None:
    ...
    x_thomas = solve_block_tridiagonal(sub, diag, sup, rhs)
    rel_err = (x_pcr - x_thomas).abs().max() / (x_thomas.abs().max() + 1e-12)
    assert rel_err < 1e-4, f"N={block_num}: PCR vs Thomas fp32 rel err {rel_err.item():.2e}"
...
def test_pcr_batched(device: torch.device) -> None:
    ...
    x_pcr = solve_block_tridiagonal_pcr(sub, diag, sup, rhs)
    assert x_pcr.shape == (*batch_shape, block_num, block_size)
