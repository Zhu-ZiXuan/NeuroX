...
class TestGeneralAdcRawConvert:
    def test_output_lies_in_raw_range(self) -> None:
        ...
        code = adc.convert(v_pos, v_neg, v_ref__V=_GENERAL_DUMMY_VREF, bits=4)
    ...
    def test_midpoint_signal_maps_to_zero_code(self) -> None:
        # Boundaries centered around 0 → signal at 0 should land at the raw
...
class TestMcsSarDifferentialVoltageAdcRawConvert:
    def test_output_lies_in_raw_range_at_max_bits(self) -> None:
        ...
        v_neg = torch.zeros_like(v_pos)
        code = adc.convert(v_pos, v_neg, v_ref__V=v_refs[..., 0], bits=4)
    ...
    def test_output_lies_in_raw_range_at_lower_bits(self) -> None:
        ...
        v_neg = torch.zeros_like(v_pos)
        # 2-bit: raw range [0, 3]
        code = adc.convert(v_pos, v_neg, v_ref__V=v_refs[..., 0], bits=2)
    ...
    def test_different_modes_use_different_v_ref(self) -> None:
        ...
        v_neg = torch.zeros_like(v_pos)
        code_mode0 = adc.convert(v_pos, v_neg, v_ref__V=v_refs[..., 0], bits=4)
        code_mode2 = adc.convert(v_pos, v_neg, v_ref__V=v_refs[..., 2], bits=4)
        # Mode 2 (tight range) saturates to raw max; mode 0 (wide range) doesn't.
        assert int(code_mode2.item()) == 15  # saturated positive → raw max
        assert int(code_mode0.item()) < 15  # within linear region


# ---------------------------------------------------------------------------
# Validation: ADC convert rejects an out-of-range bit width
# ---------------------------------------------------------------------------
...
class TestOperatingPointValidation:
    def test_mcs_sar_rejects_bits_out_of_range(self) -> None:
        ...
        with pytest.raises(ValueError, match="bits"):
            adc.convert(v, v, v_ref__V=v_refs[..., 0], bits=5)
