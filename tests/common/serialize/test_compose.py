...
from neurox.common.serialize import compose, load_config_dict
...
def test_fragment_reuse_preserves_sibling_branches_with_optional_file_suffix(tmp_path: Path) -> None:
...
def test_inline_keys_override_a_preset_chain_from_a_subdirectory(tmp_path: Path, presets_root: Path) -> None:
    (presets_root / "process").mkdir()
    _write(presets_root / "process" / "leaf.toml", "[atom]\na = 11.0\nb = 22.0\n")
    _write(presets_root / "mid.toml", '[piece]\n_neurox_use_preset = "process/leaf:atom"\n')
    _write(
        tmp_path / "main.toml",
