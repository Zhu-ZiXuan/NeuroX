...
`reference_config.values = [7.0]` (the Fig.11(a) I_LSB annotation): 224 max MAC units x 0.5 uA per unit MAC = 112 uA full scale over `2**4 = 16` codes, hence `rescale_factors = [14.0]` MAC units per final output code at `adc_bits`. That single reference is the readout's whole reference input: the RS-CSA weighs it by `2**(bits - p)` in compare phase `p`, which spans the decision ladder `c * 7.0 uA` for `c = 1 .. 15` (top decision tap 105 uA). There is no ADC calibration run — the reference follows from the paper's annotated operating point, not from a fit.
