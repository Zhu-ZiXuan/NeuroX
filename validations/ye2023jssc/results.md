...
Fixed BEFORE the C_WL sweep, and not free: `c_bl__fF` = 0.250 and `c_x__fF` = 0.150 as 28nm-class priors. They put the BL column aggregate at `bl_first_c + 63*bl_segment_c + 64*(c_bl + c_x)` = 29.57 fF, the top of the 10-30 fF estimate band, and `c_x` inside the 0.05-0.3 fF/cell band. Their energy weight is small (`.bl_cap` is 0.03 uW at the 50% point), so the C_WL solve is insensitive to where in the bands they sit.
