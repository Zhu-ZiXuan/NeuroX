...
1. **step1 — the BL/SL divider solve.** The shared nested DC solver (`NestedParallelRailSolver`) settles the T1+RRAM divider driven by the per-column BL clamp, at the level the BL converter puts on that column, against the grounded SL, yielding the per-column BL port current and `V_X`. The BL port current is what the macro bills as the input-branch conduction. The solved `V_X` is the operating point step2 reads; no energy term reads it, and it never leaves the array.
