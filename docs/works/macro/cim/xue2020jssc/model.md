...
6. **TMCSA**: `code = adc.convert(I_SUB, adc_refs_mode, bits)` against the per-instance reference ladder (`[*inst, mode, tap]` sliced by `adc_mode` to `[*inst, tap]`, broadcast against `I_SUB`); the `tmcsa` billing module then bills the conversion from `(I_SUB, code, adc_refs_mode)`; `signed = (1 - 2*sign) * code`, transposed and flattened to the primitive trailing `[col_num]`.
