...
`ADC.convert` returns signed codes in `[-2^(adc_bits−1), 2^(adc_bits−1) − 1]` (each concrete ADC owns its own conversion from native internal representation to the signed output — see [`docs/modules/analog/adc/base.md`](../../analog/adc/base.md) on the convention). The saturation mask flags a sample as saturated iff `phys_code == -2^(max_bits−1)` or `phys_code == 2^(max_bits−1) − 1` — the signed extremes carry no linear-region information.
