...
The TIA family: the abstract `TIA` (`tia/base.py`) carrying the registry, `from_config`, profiler registration, the abstract `v_ref__V` property, and the snap type parameter. Concrete impls live alongside ([opamp_tia](opamp_tia.md)). Spec: [reference/analog/tia/base](../../../reference/analog/tia/base.md).
...
- **Solver-facing surface.** `solve_clamp(i_port__uA, snap, *, v_clamp_init__V) -> (v_clamp__V, dVclamp_dI__MOhm)` plus the `v_ref__V` property - nothing else. The array solve binds the BL boundary actor to this surface abstractly; it does not depend on any concrete extension.
