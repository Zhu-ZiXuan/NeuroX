...
- **Fabricate is opt-in.** `FabricateMixin` provides the auto-cascade `fabricate()`; the default `_sample_fabricate_mismatch` is a no-op - a subclass overrides it only if it introduces static per-output mismatch.
