...
- **Two-stage non-ideality split mirrors the fabrication lifecycle.** The initial-accuracy spread is a per-die constant sampled once in `_sample_fabricate_mismatch` (stored in the `i_refs__uA` buffer); the per-read noise is resampled every `snapshot`. Same [switch_cap](switch_cap.md)-style fabricate-then-runtime split as the voltage sibling.
- **Both departures are relative (multiplicative)**, so one sigma applies uniformly across taps of differing magnitude.
