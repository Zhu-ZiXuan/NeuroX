...
- **Signed-code output, with the clamp before the shift.** Every `convert` returns a signed code in `[-2**(adc_bits-1), 2**(adc_bits-1)-1]`; the raw unsigned bucket must be clamped to its legal range *before* the zero subtraction, because stochastic-rounding jitter (floor_bucketize or SAR LSB jitter) can push it out of range and the subtraction would otherwise emit an out-of-range signed code.
