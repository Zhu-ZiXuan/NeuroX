...
- **Preset paths are forward-only.** `_validate_preset_ref_path` rejects leading `./`, any `..` segment, and absolute paths. Presets are addressed only by a forward relative path from the presets root, so a reference cannot escape the package tree.
...
N/A — load and dump are one-time setup, off every per-VMM hot path. Directive resolution caches each parsed file in a per-call `dict[Path, dict]` so a fragment referenced from several sub-tables is read once; the cache is discarded when the top-level call returns.
