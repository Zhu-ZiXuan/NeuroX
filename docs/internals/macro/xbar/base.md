...
The `XbarMacro` registry root (`xbar/base.py`): the abstract `FabricateMixin + nn.Module + ProfileMixin + RegistryMixin` orchestration node, its empty-field config/policy roots, the xbar-build helper, and the one shared geometric primitive. It declares signatures only; each mode writes its `program` / `matmul` end to end. Spec: [reference/macro/xbar](../../../reference/macro/xbar/README.md).
...
- **Empty config/policy roots.** `XbarMacroConfig` has no fields (it is only the registry key) and `XbarMacroPolicy` is an empty marker; a mode declares its own `*Config` (with `xbar_config` plus slice/reducer fields) and `*Policy` (with an `xbar: XbarPolicy` field). A composite holding a macro stores the abstract `XbarMacroPolicy` type and the caller passes the concrete impl.
