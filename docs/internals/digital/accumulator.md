...
- **`operate(x, dim)` reduces exactly one axis.** The reduced axis is gone from the output, so the serial-op divisor is just the instance count; the position-invariant numel rule (`numel(y) // inst_count`) gives the per-instance op count without re-deriving the reduced extent.
