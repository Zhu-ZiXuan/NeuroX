...
- **`stochastic_floor_div` branches on scalar-versus-tensor `rshift`.** A scalar shift draws the jitter with `torch.randint` directly in the numerator's integer dtype over $[0,\ 2^{\operatorname{rshift}})$. A tensor-valued (per-element) shift instead samples a float `uniform(0,1)`, scales by the per-element denominator $2^{\operatorname{rshift}}$ in `float64`, and casts back to the integer dtype — keeping the result exact-modulo-denominator while supporting broadcast shifts.
...
- **`boundaries` are sorted ascending of length $n_{\operatorname{codes}} - 1$.** `floor_bucketize` returns a code in $[0,\ n_{\operatorname{codes}} - 1]$. It calls `torch.bucketize(..., right=True)`: `right=True` is what produces floor semantics — a signal exactly on a boundary ($\operatorname{signal} = c \cdot \operatorname{LSB}$) lands in the upper bin and yields code $c$. The default `right=False` would round-to-nearest at boundaries; do not change it.
...
- **Stochastic-rounding jitter can overshoot the legal range.** Because jitter is added pre-floor, a value near the top boundary can produce a code one above the nominal maximum. Callers that need a bounded code (e.g. an ADC bucket index) must clamp the result themselves; the kernels do not clamp.
...
- **`right=True` is load-bearing.** Flipping `floor_bucketize` to the `torch.bucketize` default silently switches from floor to round-to-nearest at boundaries and breaks alignment with the code-edge convention $B_c = c \cdot \operatorname{LSB}$.
...
- **The tensor-`rshift` path widens to float64.** Per-element shifts pay a `float64` temporary for the denominator; a fully-integer per-element jitter would avoid it but is not implemented.
