...
- **`snapshot()` takes no external `shape`** — the output is intrinsically `(*inst_shape, mode_num, tap_num)`; a consumer indexes its mode row, selects a tap, and broadcasts it.
...
- **Tests**: `tests/primitive/analog/test_reference_sources.py`
