...
- **Family init signature.** `__init__(*, config, policy, name, inst_shape, dtype, T__K)` - all six keyword-only. The full signature lets `from_config` construct any impl uniformly; the base forwards `config` / `policy` / `name` / `inst_shape` to `AnalogBase` (which binds them and records `inst_shape`) and discards `dtype` / `T__K`, which the subclass init captures on `self.dtype` / `self.T__K`.
...
- **Calibration types.** `ADCMode` is the `(n_bits, n_states, max_signal)` record the multi-mode LUTs use; `AdcOperationPoint` is the per-call selection; `AdcCalibrationRecord` is one `(adc_mode, adc_bits) -> rescale_factor` row. Static PPA (`area_per_inst__um2`, `leakage_per_inst__uW`) lives on the family `ADCConfig`; the `ADC` base composes `ProfileMixin` to aggregate them into `area__um2` / `leakage__uW` ([base](../base.md)).
