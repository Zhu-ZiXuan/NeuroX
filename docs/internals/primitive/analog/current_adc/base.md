...
- **`convert` is a probe-emitting template method; `_convert_impl` is not `@abstractmethod`.** The base's concrete `convert` calls `self._convert_impl(...)` and then, only `if CurrentAdcProber.active()`, builds and submits a `CurrentAdcObservation` (the input magnitude, the returned `code`, and the plain `adc_mode` / `adc_bits` scalars) to `CurrentAdcProber` — demand-gated so an unsubscribed run never touches the returned code or builds the payload. `_convert_impl` raises `NotImplementedError` instead of being declared abstract — a deliberate loosening so a capture-style subclass can override `convert` wholesale and stay instantiable without a conversion body.
- **`CurrentAdcProber` is co-located with its payload in this module.** `CurrentAdcProber(Prober[CurrentAdcObservation])` sits beside `CurrentAdcObservation` in `current_adc/base.py`, next to `CurrentAdc`, the observation link's sole emitter.
