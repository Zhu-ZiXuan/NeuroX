...
- **`convert` is a record-emitting template method.** The base's concrete `convert` calls the abstract `self._convert_impl(...)` and then, only `if DiffVadcProber.active()`, builds and submits a `DiffVadcRecord` (the two input voltages, the returned `code`, and the plain `bits` scalar) to `DiffVadcProber` — demand-gated so a run nobody collects never touches the returned code or builds the record. The record covers the conversion event alone; a reference is a calibrated constant rather than a measured quantity, so it stays out of it — the same rule the current-ADC family follows.
...
- **`DiffVadcProber` is co-located with its record in this module.** `DiffVadcProber(RecorderBase[DiffVadcRecord])` sits beside `DiffVadcRecord` in `diff_voltage_adc/base.py`, next to `DiffVadc`, the family's sole emitter.
