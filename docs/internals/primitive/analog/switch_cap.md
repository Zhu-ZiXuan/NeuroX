...
- **Per-call PPA tally.** `sample_and_accumulate` self-logs a dynamic energy and a latency on every call, via `_record_dynamic_energy` / `_record_latency`. The energy is the sampled-charge term (its equation is the Reference energy model), taken on the clean sampled `v_in__V`, not the kT/C-perturbed `v_hold__V`, and reduced over the cap axis. The latency is `latency_per_op__ns * serial_op_count`, where `serial_op_count` is the position-invariant `numel // inst_count` rule taken on the *output* `v_out__V`; because `cap_num` is already summed out of `v_out__V`, the divisor is just `inst_count` and the cap axis never inflates the count.
