...
- **`cap_weights: tuple[float, ...]`**, length `n_caps`, all positive; tensor construction happens once inside `__init__` as the nominal cap-array source buffer `nominal_c__fF = c_unit__fF * cap_weights`.
- **Fabricate creates ordinary state.** `_sample_fabricate_mismatch` clone-expands `nominal_c__fF` to `(*inst_shape, n_caps)`, applies Pelgrom-scaled static mismatch (gated by `policy.cap_mismatch`), and assigns the result to ordinary `c__fF` state. The mismatch draw carries a lower floor of `0.1 * c_unit__fF` clamped onto each sampled cap, so a Gaussian tail cannot drive a sampled capacitance non-positive - the per-cap kT/C sigma and the charge-share denominator both require it positive.
