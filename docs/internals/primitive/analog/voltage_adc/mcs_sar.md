...
The SAR loop is `bits` sequential decision cycles. To keep the unrolled compiled graph short, `_convert_impl` hoists the input-independent per-bit constants out of the loop: the used-cap slices feed precomputed tables (`v_p_step_table__V`, `v_n_step_table__V`, the per-bit switch-energy tables `e_step_p_table__fJ` / `e_step_n_table__fJ`, and `c_diff_step_table__fF`), since these depend only on the fabricated caps and `v_ref` / `v_cm`, not on the runtime input. The loop body keeps only the input-dependent work — indexing the step tables by `k`, the `torch.where` top-plate updates, the comparator strobe, and the code shift — and the energy / `c_diff` side-channels are then computed vectorised from the final code bits. `_convert_impl` runs eager by default but stays compile-friendly: a caller that compiles the model then unrolls only this shortened loop. TODO - add measured per-conversion latency / memory once benchmarked.
