...
- **Family dispatch keyed on config type.** `VoltageAdc.from_config(...)` discriminates on `type(config)` through `RegistryMixin[type[VoltageAdcConfig], VoltageAdc]`; adding a concrete voltage ADC only adds a registration line and a config subclass, never touches `from_config`. `VoltageAdcConfig` is the family config base — it declares the shared static-PPA fields (`area_per_inst__um2`, `leakage_per_inst__uW`) once for every member ([base](../base.md)) and gives the polymorphic field on a parent config a base type to name.
...
- **Calibration types.** The domain-neutral `AdcMode` (the `(n_bits, n_states, max_signal)` multi-mode descriptor), `AdcOperationPoint` (per-call selection), and `AdcCalibrationRecord` (one `(adc_mode, adc_bits) -> rescale_factor` row) live in the shared `neurox/primitive/analog/adc_common.py` module and are imported by both the voltage and current ADC families.
