...
- **Zero code cached at `__init__`.** The bit width is fixed by the boundary list (single-mode), so the topology-specific midpoint `self._zero_code = n_codes // 2` is a static instance attribute committed once, not a per-call computation.
...
- **Reference-free `_convert_impl`, with `v_refs__V` deled for protocol symmetry.** `GeneralVoltageAdc._convert_impl` takes the family `v_refs__V` keyword to match the ADC base signature but discards it (`del v_refs__V`), exactly as `signed_range` `del`s `adc_bits` — its floor-bucketize is reference-free, so no tap participates in the conversion.
...
- **Single-mode validation.** `_convert_impl` accepts only `adc_mode == 0` and `adc_bits` equal to the boundary-implied width; any other operating point is rejected.
- **Clamp before the zero shift.** Per the base clamp-before-shift contract ([base](base.md)), `_convert_impl` clamps to `[0, n_codes-1]` before subtracting `self._zero_code`; here the out-of-range push comes from `floor_bucketize`'s stochastic-rounding LSB jitter.
...
- **Serial-op count.** `serial_op_count = max(1, code.numel() // max(self.inst_count, 1))` divides the total output elements by the fabrication multiplicity `inst_count = prod(inst_shape)` — fabricated instances convert in parallel, the remaining leading (batch) elements pass serially through the shared converter. GeneralVoltageAdc's `code` carries no parallel trailing dim beyond `inst_shape`, so this position-invariant numel ratio is the exact serial count; the outer `max(1, ...)` floors it at one op and `max(self.inst_count, 1)` guards a zero divisor.

## Known limitations

- `GeneralVoltageAdc` is a placeholder behavioural ADC; its comparator model, boundary list, noise stages and input-unit handling are provisional.
