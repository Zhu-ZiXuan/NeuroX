...
- **`XbarArray` is a sized `ModuleBase` + `ProfileMixin`, not a bare leaf.** Unlike the [cell](../cell/README.md) — a bare `ModuleBase` with no PPA — the array is a full electrical circuit: it is where the device children's silicon area and static leakage aggregate, so it composes `ProfileMixin` and sets the bare per-instance PPA data it aggregates into `area__um2` / `leakage__uW` (each `config.<field>_per_inst__* × inst_count`, the fields on its `XbarArrayConfig`). `ModuleBase` also gives it the `FabricateMixin` fab-cascade hook, and `ProfileMixin` the energy / latency emission.
