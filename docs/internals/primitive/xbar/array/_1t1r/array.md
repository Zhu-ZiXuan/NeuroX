...
- **The array owns a pluggable `cell` sub-module, not the devices directly.** `self.cell = XbarCell.from_config(config=config.cell_config, policy=policy.cell, inst_shape=..., dtype=..., T__K=...)` resolves the concrete cell model through the family registry from the `[cim_macro.array_config.cell_config]` sub-tree's `_neurox_class`, then asserts the result is a 1T1R-family cell (`XbarCell1t1r`) — which devices the cell owns, if any, is the cell's own business. The solver and the energy path consume the cell only through its branch / snapshot / `dynamic_energy` contract (see [cell](../../cell/_1t1r/cell.md)), so the array stamps no device current or internal node itself — this keeps it cell-agnostic.
...
- **Energy responsibility split.** `_compute_array_energy__fJ` sums the array-internal energy and delegates the per-cell node-capacitance energy to `cell.dynamic_energy(v_bl__V, v_sl__V, solver_dcop.cell, cell_snap)`, passing the converged cell DCOP so the cell charges its own node caps; the array holds no cell-cap formula. Peer blocks self-log their own driver / DAC / readout energies.
...
- **`solve_array` returns a `XbarArraySteadyState`, not a DCOP.** The frozen struct carries the reassembled `i_bl_port__uA` and `v_bl_clamp__V`, both `[..., num_line]`; energy and latency are emitted internally (`_log_dynamic_energy` + `_log_latency`), not returned. The array builds no DCOP of its own.
- **`weight_grid_shape` property.** Exposes the array-owned `self._w_layout_shape` as `tuple[int, ...]` so a caller can size a full-leading per-instance input. The array owns this shape outright (it derives it at construction and passes it to the cell as `inst_shape`); it never reads a shape back out of the cell's buffers, which may be 0-d placeholders before `program`.
