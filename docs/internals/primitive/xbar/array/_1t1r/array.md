...
- **Drivers and reference taps are injected, not held.** `solve_array(v_wl, *, bl_driver, bl_v_ref__V, sl_driver, sl_v_ref__V, t_conduct__ns)` takes the two boundary clamp drivers and their pre-snapshotted reference taps as call parameters and constructs no driver, DAC, or reference of its own. This dependency injection is what keeps the array pure.
- **`t_conduct__ns` is a scalar or a per-plane vector.** `solve_array` accepts `t_conduct__ns: float | Tensor`: a scalar for a single plane, or a per-plane vector broadcasting against the solve leading and scaling the DC-conduction energy per plane. It is converted to a tensor, broadcast to the leading dimensions, and chunk-sliced in lockstep with `v_wl`. A scalar remains 0-d. A broadcast solve over an input-bit plane axis therefore bills each plane with its own conduction window without a Python loop over planes.
...
- **`weight_grid_shape` property.** The constructor takes the standard `inst_shape` replication prefix plus `row_num` and `col_num`; `weight_grid_shape` derives the full per-cell grid shape `(*self._inst_shape, self._col_num, self._row_num)` as `tuple[int, ...]`. The array owns this concatenation and passes the result to the cell as `inst_shape`; it never derives geometry from cell buffers, which may be 0-d placeholders before `program`.
