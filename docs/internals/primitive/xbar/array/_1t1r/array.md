...
- **Compile-path: eager island.** `solve_array` carries `@torch.compiler.disable`. It owns the chunk loop, whose trip count `ceil(leading / solve_chunk_size)` is a runtime value that would unroll and explode an enclosing compiled graph if traced. Keeping it eager pins the loop in Python while the per-chunk `solver.solve_dc` — itself `@torch.compile(dynamic=False)` — compiles once at the fixed `(chunk_size, col, row)` cell-grid shape and is reused across every chunk, VMM, and caller instance. See [compile/scheme_a_regional](../../../../compile/scheme_a_regional.md).
