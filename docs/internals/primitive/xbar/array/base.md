...
- **`weight_grid_shape` property.** An init-determined `tuple[int, ...]` containing the conductance-grid shape `(*inst_shape, col, row)`. It is a property because the shape is fixed at construction.
- **`program(w_state_idx)`.** Writes the cells from one state-index tensor whose shape matches the array's own `(..., col_num, row_num)` layout; the abstract method every concrete array implements.
