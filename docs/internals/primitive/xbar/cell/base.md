...
- **The cell remains in the module tree without reporting PPA.** Device children require module registration so buffers follow `.to(...)` / `.eval()` and fabrication reaches them. `is_profile_target = False` excludes the cell from static collection and forbids direct dynamic-event emission; `dynamic_energy(...)` returns the per-cell value instead. `XbarCellConfig` therefore carries no shared PPA fields.
...
- **Abstract method set.** Every implementation supplies `snapshot`, `program`, `solve_branch`, `solve_dc`, and `dynamic_energy`. `solve_branch` returns only `(i__uA, di_dvbl__uS, di_dvsl__uS)`; `solve_dc` returns the corresponding `DCOPT` superset. Their branch triples must be identical for the same inputs and snap. `dynamic_energy` consumes that DCOP without mutating cell state.
