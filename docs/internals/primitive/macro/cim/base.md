...
- **`active_row_num` is range-guarded only.** `CimMacroConfig.validate()` enforces `1 <= active_row_num <= row_num` and imposes no divisibility requirement. `max_active_rows` exposes the configured limit.
