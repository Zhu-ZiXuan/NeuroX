...
- **Static PPA surface.** `CimMacro` is a sized `ModuleBase[CimMacroConfig, CimMacroPolicy]` + `ProfileMixin` + `RegistryMixin`. `CimMacroConfig` carries `area_per_inst__um2` / `leakage_per_inst__uW` (with `validate_ppa`), and the tile sets the bare per-instance PPA data that `ProfileMixin` aggregates into `area__um2` / `leakage__uW` (`config.<field>_per_inst__* × inst_count`) for the profiler's static walk ([base](../../../common/base.md)).
