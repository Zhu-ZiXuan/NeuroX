...
- **`active_row_num` is range-guarded only.** `CimMacroConfig.validate_geometry` enforces `1 <= active_row_num <= row_num` and imposes no divisibility requirement. `max_active_rows` exposes the configured limit.
...
- **Abstract value-domain and ADC surface.** Each subclass implements the value-domain properties (`x_range`, `w_digit_count`, `w_digit_radix`, `w_digit_range`) and the ADC operating-point surface (`adc_mode_num`, `adc_max_bits`, `adc_rescale_factor`). `adc_rescale_factor(*, adc_mode, adc_bits)` returns the code-to-dot-product rescale keyed on the `(adc_mode, adc_bits)` pair passed as two keyword-only ints, and raises `KeyError` for an uncalibrated operating point.
