...
- **Cross-module snap passing is allowed and is not a new convention.** A solver may iterate on snaps owned by other modules (`NestedSolver.solve_dc` reads the cell snap carrying `RRAMSnap` / `NMOSSnap`, and two clamp-driver snaps). The snap type carries its own semantics; the receiver's signature documents which type it expects. No extra rule beyond the snap contract above applies.
