...
Every fabricated circuit or device carries its physical state through three explicit stages — nominal (design intent), actual (post-fabrication / post-programming), snapshot (per-call dynamic noise) — each with its own representation, write moment, and name form. This document covers the state-ownership and snapshot contracts that hold across the module tree; the call lifecycle that drives the stage transitions is in [fabrication_lifecycle](fabrication_lifecycle.md).

## Design decisions

- **Three stages, not two.** Splitting actual from snapshot keeps the solver iterating against a frozen electrical characteristic: actual state (static mismatch) is resamplable but stable for the lifetime of a fabricated module, whereas snapshot state (kT/C, RTN, comparator decision noise) is the value the circuit sees at one instant of one DC solve. Folding dynamic noise into the actual buffer would let the characteristic drift between Newton iterations of the same call; folding mismatch into the snapshot would re-pay the sampling cost every call and lose the resample-once semantics. Rejected — a single mutable "current value" buffer (no clean resample boundary, aliasing across calls); chosen — three named tiers with distinct write moments.
- **The snapshot is a frozen dataclass, never a buffer.** A `@dataclass(frozen=True)` returned per call cannot be reassigned mid-solve and is discarded with the call, so no per-call state leaks onto the module and `state_dict` stays clean. Registering it as a buffer would persist call-local noise and invite the aliasing the three-stage split exists to prevent.
...
- **Cross-module snapshot passing is allowed and is not a new convention.** A solver may iterate on snapshots owned by other modules (`Solver1T1R.solve_dc` reads `RRAMSnapshot`, `NMOSSnapshot`, and two clamp-driver snapshots). The snapshot type carries its own semantics; the receiver's signature documents which type it expects. No extra rule beyond the snapshot contract above applies.
- **Per-call runtime state never reattaches to the module.** Anything created inside `solve_dc(...)` / `convert(...)` / `vec_mat_mul(...)` other than the snapshot is held in locals, returned via the `*DCOP` / `*Output` / output tensor, or emitted on the profiler side channel — never reassigned to a module attribute.

## Performance & resources

- The snapshot is allocated per call at the per-call broadcast `shape`, so dynamic-noise memory scales with the call's leading batch, not with `inst_shape`. It is freed when the call returns; nothing in the three-stage model retains per-call tensors. Resample cost (the actual stage) is paid only on `fabricate()`, and sampling reads the unchanged nominal buffers, so repeated fabricate calls do not grow memory.

## Gotchas

- **Do not treat the snapshot as durable state.** It is valid for exactly one DC solve. Caching a snapshot across calls reintroduces the mid-iteration drift the frozen-per-call rule forbids, and stale noise will silently bias a later solve.
- **`shape` (snapshot) is not `inst_shape` (fabrication).** The snapshot `shape` is the per-call broadcast shape; `inst_shape` is the construction-time per-instance multiplicity used to resample the actual value. Passing one where the other is expected mis-sizes the noise tensors.
