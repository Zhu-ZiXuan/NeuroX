            ...
            -> ChunkedSolver.solve_dc  @torch.compiler.disable(recursive=False)   # eager island
                 for chunk in iter_chunks(...):           # Python loop, never in a graph
                     solver.solve_dc(chunk)               # eager wrapper: probe guard only
                       -> _solve_dc_compiled(chunk)       # the compiled leaf  <- the only self-compiled region
...
solver._solve_dc_compiled   @torch.compile(dynamic=False) # fixed chunk shape -> one shared graph
```
...
- **Regional leaf** — the concrete solver's solve body (`NestedParallelRailSolver._solve_dc_compiled`, `@torch.compile(dynamic=False)`); its `solve_dc` entry point is an undecorated eager wrapper holding the probe guard, as is the abstract `Solver.solve_dc`. The **only** region the library self-compiles. Shape-stable: the chunk slicing flattens every call to one `(chunk_size, col, row)` cell-grid shape. Obeys the contracts.
...
- **The eager island suppressing the leaf compile.** If a PyTorch version makes the disabled chunk loop propagate "do not compile" into its callees, `_solve_dc_compiled` would silently run eager and the bottleneck would never compile — no error, just slow. This is the recursive-disable edge above; it must be re-confirmed (e.g. that the leaf produces exactly one compiled graph) whenever the PyTorch version changes.
