...
- **Do not reconstruct fixed tensor sources on the run path.** A config- or design-derived tensor constant is registered at construction and moved with the module before state materialization. Runtime-derived tensors use a semantically related input and `*_like` / `new_*` operations. Never use an unrelated tensor merely as a dtype/device anchor, and never rebuild a fixed Python scalar or sequence with `torch.tensor(..., device=...)` inside the compiled path.
