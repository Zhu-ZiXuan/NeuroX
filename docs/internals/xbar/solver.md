...
- **Block-count semantics ($B$).** `solve_block_tridiagonal` works for any block size $B$ from the same code path: $B=1$ reduces to scalar Thomas and matches `solve_tridiagonal` numerically up to fp64 round-off; $B=2$ is a coupled two-rail wire Newton; $B=3$ is a monolithic full-Jacobian Newton step. The single-block fast path ($N=1$) is one $B \times B$ `torch.linalg.solve`.
...
- **Sign / unit conventions.** The KCL residual is returned in uA with the convention $r = i_\text{inject} + \Delta v_\text{left} \cdot g_\text{left} + \Delta v_\text{right} \cdot g_\text{right}$, where $\Delta v_\text{left}[k] = v[k] - v[k-1]$ (and $v[0] - v_\text{drive}$ at the driver end) and $\Delta v_\text{right}[k] = v[k] - v[k+1]$ (zero at the far end, which has no right segment). Node voltages are V, segment conductances uS, currents uA.
