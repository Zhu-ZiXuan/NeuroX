...
- **Snap per chunk.** Each chunk builds one `cell_snap` (`self.cell.snapshot(...)`) plus the per-chunk snaps of the passed-in BL/SL drivers, all at the chunk shape inside the loop; the core re-registers no child state and constructs no driver.
