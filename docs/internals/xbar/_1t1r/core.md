...
- **`weight_grid_shape` property.** The core exposes `weight_grid_shape -> tuple[int, ...]` (the `self.cell.rram.g__uS.shape`) so the scheme xbar can expand `x` to full leading before its WL DAC convert — preserving the per-instance drive-noise behavior.
...
- **Reference**: [Core1T1R](../../../reference/xbar/_1t1r/core.md)
