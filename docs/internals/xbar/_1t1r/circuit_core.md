...
- **Snapshot per chunk.** Each chunk builds one `cell_snapshot` (`self.cell.snapshot(control=v_wl_chunk, ...)`, which bundles the RRAM / NMOS device snapshots with the WL drive) plus the TIA / SL-driver snapshots, all at the chunk shape inside the loop; the core does not re-register child state.
