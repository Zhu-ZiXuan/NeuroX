...
- **`to_ideal()` carries `XbarConfig` fields only** — never a physical subclass's calibration table, which lives on the physical config; the ideal twin must derive its rescale from geometry alone.
