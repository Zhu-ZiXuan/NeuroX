...
- **`config` is the typed PPA source.** `config: ConfigT` is bound to a frozen dataclass whose type inherits `CircuitConfig`, which supplies the two base fields `area_per_inst__um2` and `leakage_per_inst__uW`. The generic parameter (`VoltageDriver(CircuitBase[VoltageDriverConfig])`) narrows `self.config` so subclass fields type-check without a per-leaf forward declaration; an `nn.Module` whose `__getattr__` blurs the narrowed type may still carry a one-line class-level `config: <LeafConfig>` annotation.
