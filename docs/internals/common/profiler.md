...
`NeuroxProfiler` (`common/profiler.py`) is the context manager that collects the PPA side channel: during its `with` block it captures the per-call dynamic energy / latency events that leaves emit, walks `CircuitBase` instances for static area / leakage, and aggregates both into a `ProfilerReport`. It is the collector half of the mechanism — the per-module emitter is [mixin/profile](mixin/profile.md) — and how each PPA quantity is physically computed is a per-subsystem cost model under [reference](../../reference/README.md).
...
- **One energy event + one latency event per logical operation.** A plain leaf emits inline once at the end of forward. A composite whose body contains an internal chunked / iterated loop must still emit exactly once per operation: aggregate per-chunk inside the loop, then make the two emit calls once after it. Sub-solvers and devices inside the loop are not `CircuitBase` and emit nothing, so there is no double-count. A scheme's chunking test covers the single-event invariant.
- **Serial-op count is generic; per-leaf overrides live with the leaf.** A leaf's latency tensor scales by a serial-op count `serial = max(1, numel // inst_count)` — output elements over the parallel fabrication multiplicity ([`circuit.md`](circuit.md)). A leaf whose output carries extra physically-parallel trailing axes divides by a larger parallel count than `inst_count`; that exact derivation is part of the leaf's cost model and lives in its `reference/<subsystem>/` page.
...
- **The serial-op count, not the cost model, is the leaf's responsibility.** The collector only requires a tensor; how a leaf derives its serial-op multiplicity — the generic `numel // inst_count` or a leaf-specific override — is part of each leaf's cost model and is specified in its `reference/<subsystem>/` page.
