...
- **Static collection keys on the reporting flag.** Every module is a `ProfileMixin` host carrying the `area__um2` / `leakage__uW` surface, so being a host does not imply owning reportable silicon: the walk keys on `reports_static_ppa` and reads the two properties from the hosts that declare it true. That selects exactly the modules accounting for themselves — a composite reports only its own overhead while its children self-report, and a class whose area is already inside an owner's budget declares the flag false and stays out, so nothing is double-counted. The reporting boundary is in [base](base.md).
...
- **The serial-op count belongs to the leaf, not the collector.** A leaf hands over a latency tensor already scaled by its serial-op count — generically `max(1, numel // inst_count)`, output elements over the parallel fabrication multiplicity `inst_count` ([base](base.md)), or a leaf-specific override. That derivation is part of the leaf's own cost model, in its `reference/<subsystem>/` page; the collector only requires the tensor.
