...
`NeuroxProfiler` (`common/profiler.py`) is the context manager that collects the PPA side channel: during its `with` block it captures the per-call dynamic energy / latency events that leaves emit, walks the `ProfileMixin` instances that carry static area / leakage, and aggregates both into a `ProfilerReport`. It is the collector half of the mechanism — the per-module emitter is the profile mixin — and how each PPA quantity is physically computed is a per-subsystem cost model.
...
- **No static-walk cache.** `collect_static` re-walks the graph on every report rather than memoizing the result. The per-module area / leakage properties it reads are already cheap ([mixin/profile](mixin/profile.md)), so a walk cache would add an invalidation surface for no measurable saving.
- **Static collection keys on `isinstance(m, ProfileMixin)`.** `ProfileMixin` owns the static area / leakage aggregation — each property scales the per-instance data a sized leaf sets by `inst_count` ([mixin/profile](mixin/profile.md)) — so walking it collects exactly the modules that report silicon; a composite reports only its own overhead while its children self-report, so nothing is double-counted. A bare `ModuleBase` leaf (a device, the current mirror / mux, an xbar cell) composes no `ProfileMixin` and the walk skips it. The membership boundary — which modules are bare vs sized — is in [base](base.md).
...
- `analyze_static(model) -> StaticMetrics` — static-only staticmethod; aggregates the `ProfileMixin` walk to totals. `StaticMetrics` carries area + leakage only (`area__um2`, `leakage_power__uW`), no dynamic quantity.
