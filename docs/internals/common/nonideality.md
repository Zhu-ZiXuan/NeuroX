...
- **Pelgrom mismatch sigma direction.** The per-cell mismatch sigma in `apply_pelgrom_mismatch` grows with cell area (an absolute-spread framing), whereas the canonical Pelgrom relative mismatch shrinks with area. Whether the kernel realises the intended framing is unresolved and shared with the reference. TODO — domain author to confirm the sigma direction against [reference/nonideality](../../reference/nonideality.md).
