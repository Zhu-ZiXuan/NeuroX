...
- **The chunking layer deliberately does not dispatch on it.** Chunk slicing classifies its arguments by a structural predicate — a frozen dataclass is a snap — and walks whatever it finds with `walk_tensor_fields` directly. Requiring the mixin instead would make chunkability an inheritance decision: a payload authored elsewhere, or one that never needed the shape surface, would silently stop being sliced. The traversal is the shared thing; the mixin is a convenience on top of it, and the layer below takes the traversal.
- **Inheritance follows use, not kind.** A frozen dataclass of tensors carries the mixin where something actually reshapes the group; a payload that only ever travels whole does not, because inheriting a surface nobody calls only widens the type's public API.
...
- **Implementation**: `neurox/common/mixin/tensor_group.py`
