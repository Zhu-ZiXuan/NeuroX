...
Registration is an import side-effect: a concrete implementation registers itself when its module is first imported. Because a family root package imports its concrete modules, importing that package completes registration of the whole family; the registration and lookup mechanism is specified in [registry](common/mixin/registry.md). A family package therefore does not need to be re-exported by its parent for its registrations to complete. No automated check verifies that a package's `__init__.py` imports keep its public surface — and therefore its registrations — complete.
