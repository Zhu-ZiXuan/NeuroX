...
- **`conv2d` matches `F.conv2d` shape semantics.** Trailing `[C_in, H, W]` maps to trailing `[C_out, H_out, W_out]`; every leading dim is a broadcast batch dim that rides through untouched. Grouped convolution is unsupported. The bias adds as `_int_bias.view(-1, 1, 1)` over the output map, after the fold, in the int64 accumulation domain.
