...
- **`_int_bias` is programmed state, not a buffer.** The class-scope default is `None`; `_program_int_bias` stores the validated int64 tensor as an ordinary attribute, and `bias=None` clears it. It therefore follows the project lifecycle: move the unit before programming, and reprogram after any later migration.
...
- **The quantization surface is published, not enforced.** `adc_max_bits` advertises the maximum resolution and `rescale_factor(*, quantization_mode, adc_bits)` the code currency of one operating point; the substrate below owns which modes exist and rejects an undeclared one. `adc_max_bits is None` means the unit never quantizes its output — not a one-level ADC — and `adc_bits is None` requests the matching lossless read, so the whole chain from the operator down to the substrate carries `int | None`. Value ranges are published capability, not a base-enforced runtime bound.
