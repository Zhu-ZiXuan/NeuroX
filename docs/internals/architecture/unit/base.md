...
- **`int_bias` is programmed state, not a buffer.** The class-level default is `None`; `_program_int_bias` stores the validated int64 tensor as an ordinary attribute, and `bias=None` clears it. It therefore follows the project lifecycle: move the unit before programming, and reprogram after any later migration.
