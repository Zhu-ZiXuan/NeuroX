...
The cross-plane `Sw` mode. It owns a tile, a `SimpleSlicer` (weights), a `SerialSlicer` (activations), and three reducers — an `Accumulator` (`Tc`) plus two `ShiftAdder`s (`Sa`, `Sw`).
