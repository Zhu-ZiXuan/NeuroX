...
- **Uniform row-blocking is enforced here, not at the macro.** `LinearCimUnitConfig.validate()` requires `row_num % active_row_num == 0` on the owned CIM macro. The linear operator reads every row of the weight matrix, so the engine's sub-phases must tile `row_num` into equal `active_row_num` blocks — each sub-phase then carries the same dot-product dynamic range (hence the same ADC calibration). The base `CimMacroConfig` deliberately omits this divisor (a non-divisible tile is valid for operators that tolerate a short final block, e.g. `Conv2dCimUnit`); the constraint lives at the operator layer that actually needs it.
