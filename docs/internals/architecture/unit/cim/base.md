...
- **Engine policy assembled in code.** `EngineBackedCimUnitPolicy` keeps the `cim_macro` field flat; the unit constructs `CimEnginePolicy(cim_macro=policy.cim_macro)` itself, so policy TOMLs carry no engine nesting.
