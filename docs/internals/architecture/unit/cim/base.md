...
The `CimUnit` registry root: a `FabricateMixin + nn.Module + ProfileMixin + RegistryMixin` orchestration node holding the empty-field config/policy roots, the `_build_cim_macro` helper, and the `chunk_pad_along` primitive. It declares the `program` / `matmul` signatures only; each mode writes them end to end.
...
- **Empty config/policy roots.** `CimUnitConfig` has no fields (it is only the registry key) and `CimUnitPolicy` is an empty marker; a mode declares its own `*Config` (with `cim_macro_config` plus slice/reducer fields) and `*Policy` (with a `cim_macro: CimMacroPolicy` field).
