...
- **Config carries the unit-local PPA; policy root is empty.** `CimUnitConfig` carries and validates the two required peripheral-PPA fields `area_per_inst__um2` / `leakage_per_inst__uW` and is the registry key; `CimUnitPolicy` is an empty marker. `EngineBackedCimUnitConfig` adds `engine: CimEngineConfig`; `EngineBackedCimUnitPolicy` adds the parallel `engine: CimEnginePolicy` dispatch point.
