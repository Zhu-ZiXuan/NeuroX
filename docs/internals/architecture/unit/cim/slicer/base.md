...
The slicer layer is the `Slicer` ABC plus three concrete members: `DirectSlicer` (the identity member for values already on the macro's per-cycle grid), `SerialSlicer` (the bit-serial activation path), and `SimpleSlicer` (the weight path). The latter two each wrap a [transcoder](../../../../common/encoding/README.md) and regroup its output into the slice/digit lattice; `DirectSlicer` wraps nothing and decomposes nothing — it validates the value range and appends the structural axes.
...
- **Output shape contract.** `DirectSlicer.slice(x).shape == x.shape + (1, 1)` (both trailing axes structural singletons; values and dtype unchanged); `SerialSlicer.slice(x).shape == x.shape + (slice_num, 1)` (structural singleton digit axis); `SimpleSlicer.slice(x).shape == x.shape + (slice_num, digit_count)`. The slice axis is least-significant-first, matching `slice_weights`.
...
- **Digit-grid compatibility is the caller's invariant.** The slicer does not check that its `[slice_num, digit_count]` lattice matches a particular cell grid; the construction site (which forwards the cell's digit count, digit radix, and input grid into the right subclass) owns that match.
...
- **Out-of-range handling differs per member.** For the transcoder-backed pair the value range is a caller contract, not a clamp: an input above the band wraps silently through the encode. `DirectSlicer` is the exception — it checks elementwise and raises `ValueError` on any out-of-range value.

---

- **Reference**: [slice decomposition](../../../../../reference/architecture/unit/family.md)
- **Implementation**: `neurox/architecture/unit/cim/slicer/base.py`, `neurox/architecture/unit/cim/slicer/direct.py`, `neurox/architecture/unit/cim/slicer/serial.py`, `neurox/architecture/unit/cim/slicer/simple.py`
