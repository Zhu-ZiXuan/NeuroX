...
- **The unit owns the two-stage digital accumulation over the macro's per-phase codes.** The macro emits per-phase ADC codes with trailing `[active_phase_num, col_num]`; the unit reduces the active-phase axis at each tile's output port (`phase_accumulator`, a `SerialAccumulator` — the phase axis is time-serial on one register per tile port, so it bills per arriving per-phase code), then the `Tc` tile axis at the unit (`col_accumulator`, a parallel `Accumulator` billed per output element). Different reduction axes are different circuit modules — separately fabricated, separately billed — never merged into one reduce.
