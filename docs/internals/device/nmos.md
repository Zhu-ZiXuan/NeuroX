...
`NMOS` (`nmos.py`) is a stateless-in-conduction electrical primitive: it precomputes temperature-scaled nominals and Pelgrom sigmas at construction, resamples per-cell `beta__uA_per_V2` / `vth__V` at fabricate time, and evaluates the EKV-softplus I-V and its three node partials in closed form. It realizes the model in [reference/device/nmos](../../reference/device/nmos.md). This document covers the non-obvious choices, not the I-V flow.

## Design decisions

- **NMOS is a pure electrical primitive — no layout parasitics.** Layout-dependent lumped capacitances are deliberately excluded; the access-transistor parasitics belong to the circuit that places the transistor, which carries its own design parameters and computes its own effective-capacitance scalars. Pushing them into the device would couple a foundry primitive to a specific cell layout.
- **`W__um` / `L__um` are `__init__` kwargs, not config fields.** Channel geometry is a device design parameter chosen per placement, so it stays out of the frozen `NMOSConfig` (which holds process electricals and the Pelgrom matching coefficients only) per the project rule.
...
- **Policy is separate and per-call-site constructed.** `NMOSPolicy` (two flat `bool` mismatch flags, no defaults) is loaded from its own file; with a flag off the corresponding `apply_gaussian` is the identity, so noise-off runs allocate no mismatch draw.
...
- **`snapshot(shape, multi_coords)` is the read path into fabricated state.** It expands the per-cell maps to the per-call `shape`, optionally advanced-indexes a chunk via `multi_coords`, and returns an `NMOSSnapshot`. The solve reads `$\beta$` and `V_th` only from the snapshot, never from `self`.
