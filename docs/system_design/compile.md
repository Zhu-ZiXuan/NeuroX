...
- **Eager island** — `execute_chunked`, the only chunk loop in the library; every chunked array reaches a solve through it and none holds a loop of its own. It carries `@torch.compiler.disable(recursive=False)`, so it stays eager even inside a caller's compiled model while the injected solver leaf still compiles. It is the one place a runtime trip count, per-chunk allocation release, and host-side accumulation are legal.
- **Compiled leaf** — `_solve_col_bl_col_sl_dc_impl` under `@torch.compile(dynamic=False)`, the only region the library compiles for itself. Its `solve_col_bl_col_sl_dc` entry point stays undecorated so the prober gate is read outside the graph.
