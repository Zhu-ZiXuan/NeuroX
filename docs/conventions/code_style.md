...
- Function defaults express a no-op, neutral element, or absent state; public constructors and factories may also default to a recommended computation dtype. Other behavior, policy, and physical-value parameters remain required. This exception does not apply to config or policy fields.
