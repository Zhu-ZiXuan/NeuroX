...
- Keep the primary data input positional-or-keyword. A short operand sequence with a conventional order may also precede `*`; setters and single-value queries need no keyword boundary merely because their operand is a scalar.
- Put configuration, execution controls, reference values, bounds, and optional auxiliary inputs after `*`. Required parameters can be keyword-only: importance and the presence of a default do not decide the boundary. Constructors that bind several configuration and runtime-context values use named arguments throughout.
- Require names for same-typed arguments whose distinct roles make a swap difficult to detect, such as electrical terminals and physical quantities with different units. A bare `Tensor` or `float` annotation cannot distinguish these roles. Keep callbacks and state named where their order has no established convention.
- Keep keyword boundaries consistent across protocols, base declarations, overrides, and overloads, and update their callers together. Express the boundary in the signature so Python and static checkers enforce it directly.
