...
- **Static (leakage) energy** integrates over the measurement or clock period used by the relevant validation or deployment model, not the conduction span. A boundary exposing `latency__ns` reports one complete public operation, including its internally serialized work; a paper's measurement period remains validation data unless that scheduling behavior is explicitly modeled.
