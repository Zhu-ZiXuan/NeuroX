...
9. [compile]     Primary methods on the compile path; do not self-decorate with `@torch.compile` → `compile_policy.md` §Where the compile boundary lives
...
11. [doc]        Write the `reference/<subsystem>/<name>.md` and `internals/<subsystem>/<name>.md` per the templates; include `Compile-path: yes/no` on each primary method → [writing_reference_docs](writing_reference_docs.md), [writing_internals_docs](writing_internals_docs.md), `compile_policy.md` §Per-method declaration
