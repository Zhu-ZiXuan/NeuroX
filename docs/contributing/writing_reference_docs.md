...
- A directory's abstract layer (its direct files) is the directory-level document(s); subdirectories recurse the same way. Example: `xbar/` = `base.md` (abstract `Xbar` + ideal twin) + `cell.md` (abstract cell) + `solver.md` (the `solver/` package merged) + `_1t1r/` (topology family: `cell`, `circuit_core` shared array, `offset` operating xbar) + `readout/`. Adding `_2t1r/` only adds a sibling subdirectory and leaves `base.md`, `cell.md`, and `solver.md` untouched — mirroring "add the `_2t1r/` dir, leave the abstract layer alone" in code.
