...
- **Merge only the direct files of one directory** when they describe one coherent topic (e.g. `_1t1r/`'s three solver files become one `solver.md`); never merge across directories, never collapse a subdirectory.
- A directory's abstract layer (its direct files) is the directory-level document(s); subdirectories recurse the same way. Example: `xbar/` = `base.md` (abstract `Xbar` + ideal twin) + `_1t1r/` (topology family: `circuit_core` shared array, `solver`, `offset` operating xbar) + `readout/`. Adding `_2t1r/` only adds a sibling subdirectory and leaves `base.md` untouched — mirroring "add the `_2t1r/` dir, leave `base.py` alone" in code.
