...
- **Control line carried in the snapshot.** The per-call cell snapshot bundles the device snapshots with the cell's control-line drive (the word line for 1T1R), so the solver passes one `cell_snapshot` and holds no cell-internal state.
