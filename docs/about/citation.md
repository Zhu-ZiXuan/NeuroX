...
The supported public surface of NeuroX stops at `neurox.macro`. The device, crossbar, analog, digital, mapper, and profiler layers below it are implementation detail: their classes, configs, and signatures may change without a deprecation cycle. Build against `neurox.macro` (and the example flows under `example/`) for any code you intend to keep working across releases; reach below it only with the understanding that it is unstable by design.
