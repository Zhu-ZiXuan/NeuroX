...
All tools are **config-driven**: each takes a single `--config <run.toml>` plus a handful of runtime / output flags. Every input that affects the result lives in the TOML — chip preset (typically wrapping a scheme's params + all-off policy preset via a `_neurox_use` directive in the config section the tool's schema names), workload, sweep ranges, RNG seed, dtype. The CLI carries only runtime / output knobs:
