...
- **`--policy`** — the mutable nonideality switches. The example policy files reference the all-off preset shipped with the scheme via `_neurox_use`, so every nonideality (device mismatch, thermal noise, programming noise, ADC offsets, ...) is off by default. To enable one, override the matching switch inline:
  ...
  The solver chunking knob (`solve_chunk_size`) also lives here, under `[policy.cim_macro.array]`.

## See also

- [API: configuration](../../api/configuration.md) — the `--config` / `--policy` TOML schema and presets.
- [Reference: architecture/unit](../../reference/architecture/unit/README.md) — the `CimUnit` family and `from_config` factory.
- [Reference: xbar solver](../../reference/primitive/xbar/solver/README.md) — the topology-agnostic physical-array solve: the single nested formulation over the pluggable cell and clamp drivers.
