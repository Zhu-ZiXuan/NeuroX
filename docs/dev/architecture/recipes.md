...
4. [init]        Standard family signature `__init__(self, *, cfg, name, T__K, dtype)`; convert config units in `__init__` → [`physical_units.md` §Config units](physical_units.md)
...
6. [fabricate]   `fabricate(self, shape, **extras) -> None` populates actual-value buffers from nominals + static mismatch, then `self._record_inst_count(shape)` → [`fabrication_lifecycle.md` §Canonical signature](fabrication_lifecycle.md), [`profiler_and_ppa.md` §`_record_inst_count`](profiler_and_ppa.md)
...
3. [class]       `class <Name><Family>(<Family>)`; register via `@<Family>.register_config(<Name><Family>Config)` → [`config_and_construction.md` §Family bases](config_and_construction.md), [`ADR-0001`](../adr/ADR-0001-config-dispatch-and-owned-construction.md)
...
5. [fabricate]   Implement `fabricate(self, shape, **family_extras)`; end with `self._record_inst_count(shape)` → [`fabrication_lifecycle.md` §Canonical signature](fabrication_lifecycle.md)
...
1. [adr]         Write an ADR explaining why the new family is needed and what alternatives were rejected → [`adr/README.md`](../adr/README.md)
