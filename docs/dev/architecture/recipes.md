...
4. [init]        Use the family-wide signature (`cfg, name, inst_shape, dtype, T__K` + family-specific extras like `ideal_xbar`); no defaults on any kwarg; call `super().__init__(...)` → [`config_and_construction.md` §Family-wide init](config_and_construction.md), [`code_style.md` §Physical-layer no defaults](code_style.md)
