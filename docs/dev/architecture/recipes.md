...
11. [doc]        Write `docs/modules/device/<name>.md` describing the device's current responsibility and protocol surface → [`code_style.md` §Module docs](code_style.md)
...
11. [doc]        Write `docs/modules/<path>/<name>.md`; include `Compile-path: yes/no` on each primary method → [`compile_policy.md` §Per-method declaration](compile_policy.md)
...
9. [doc]         Write `docs/modules/<path>/<name>.md`; cross-reference the family base in `See also:` → [`code_style.md` §Module docs](code_style.md)
...
5. [docs-base]   Write `docs/modules/<path>/base.md` (or family `README.md`) describing the protocol surface in abstract terms (no specific consumer names) → [`code_style.md` §Documentation dependency direction](code_style.md)
...
3. [register]    For a registry-dispatched family (Transcoder): add `@<Family>.register_key("<discriminator>")` on the concrete subclass; for direct-instantiation families (Slicer): omit this step → [`docs/modules/common/registry_dispatch.md`](../../modules/common/registry_dispatch.md)
...
7. [doc]         Write `docs/modules/mapper/.../<name>.md` → [`code_style.md` §Module docs](code_style.md)
