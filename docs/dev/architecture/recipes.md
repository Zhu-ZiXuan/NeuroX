...
4. [init]        Standard family signature `__init__(self, *, config, name, inst_shape, dtype, T__K)` — every kwarg is required, no defaults anywhere in the physical layer; convert config units in `__init__`; set `self._inst_shape = inst_shape` and call `self._log_static()` once at the end after `self.config` is set → [`physical_units.md` §Config units](physical_units.md), [`code_style.md` §Physical-layer no defaults](code_style.md), [`profiler_and_ppa.md` §`_log_static`](profiler_and_ppa.md)
