...
- a per-instance `AnalogMuxPolicy` carrying `mux_noise_cm: bool` and `mux_noise_dm: bool` toggles
...
`__init__(*, config, policy, name, inst_shape, dtype, T__K)`. The per-instance count is locked from `inst_shape` at construction.
