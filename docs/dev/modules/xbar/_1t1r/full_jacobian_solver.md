...
Both solvers benefit from `torch.compile` (kernel fusion + dropped intermediates). For debug / cross-check (only), switching to FullJacobian is a one-line edit of the `_neurox_type` discriminator in `[xbar.core_config.solver_config]`. The two solvers are cross-checked against each other in `test_full_jacobian_solver.test_full_jacobian_matches_nested`.
