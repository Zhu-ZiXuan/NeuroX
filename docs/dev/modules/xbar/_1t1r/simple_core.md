...
`simple_core.py` is reserved for a lighter, behavioural 1T1R core that will not run the full Newton-Raphson solver. It is intended for fast sanity checks and large-array smoke tests where the analog non-idealities covered by `CircuitCore1T1R` would dominate the wall-clock budget without materially changing the correctness signal.
