...
4. `vec_mat_mul(x, *, adc_operation_point)` widens digits / weights / activations to `int64`, collapses the digit axis with the radix-derived `digit_weights` vector `(1, r, r², …, r^(D-1))`, runs an exact integer dot-product, and — for `adc_operation_point.adc_bits > 0` — floor-quantises with the LUT's `rescale_factor` then clamps to the signed `adc_bits` range. `adc_bits == 0` is a sentinel: the lossless integer dot product is returned unmodified, matching `IdealXbarMacro`.
