...
- **Lifecycle** — a constructed unit is an `nn.Module`, so `.to(device)` and `.eval()` apply as usual; `fabricate()` resamples the static manufacturing variation of the unit and everything below it. What each call settles is in [physical state](../system_design/physical_state.md).
...
- **Module root** — `neurox.common.ModuleBase`, with the immutable value objects `ConfigBase` and `PolicyBase` it is constructed from. It supplies the config and policy properties, the instance multiplicity, the `fabricate()` traversal, and the static PPA hooks a profile target answers.
