# Inter-array slice unit

This mode spends one tile plane per weight slice to widen the weight value range beyond a single tile's value domain, recombining the planes by a cross-plane positional shift-add.
...
N/A at the mode level. ADC quantization and analog non-idealities enter through the per-plane tile reads, specified in [CimMacro base](../../../primitive/macro/cim/README.md); both shift-adds and the contraction accumulation are exact integer arithmetic.
...
- The mode trades tile count for value-domain reach: $S_w$ planes are spent to widen the weight range.
...
- **Internals**: [inter_array_slice internals](../../../../internals/architecture/unit/cim/inter_array_slice.md)
