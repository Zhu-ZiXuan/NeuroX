...
The 1T1R cell is the concrete realization of the [cell family contract](../cell.md): the two-terminal element between a bit-line node and a source-line node. It models one RRAM device in series with an access NMOS, $\mathrm{BL} - \mathrm{RRAM} - V_{\mathrm{X}} - \mathrm{NMOS} - \mathrm{SL}$, with the NMOS gate driven by the word line. It presents a single condensed branch: the cell solves its own internal access node $V_{\mathrm{X}}$ and exposes one branch current with two signed terminal conductances. This document specifies the cell's physical model, the access-node condensation, the signed-conductance contract of its condensed branch, and its device-capacitor energy; the array-level wire ladder, boundaries, and array energy are out of scope here.
...
| `rram_g_max__uS` | maximum programmable RRAM conductance | uS | Design |
| `state_to_g_map__uS` | state-index to target conductance (strictly increasing) | uS | Calibrated |
...
**Validation constraints.** The conductance map must be physically consistent with the RRAM window and the device floor $G_{\mathrm{min}}$ (`rram_config.g_min__uS`):
...
The wire-segment, control-line (WL), and DC-conduction energy is **not** the cell's — it is an array-level contribution. The cell carries no other PPA: its device children's silicon area and leakage are accounted at the array level, not by the cell.
