...
The static PPA follows from the same lattice. Its silicon area is $N_{\mathrm{row}} N_{\mathrm{col}}$ seats at one row pitch by one column pitch. Static leakage is zero because either scan organization puts every cell at zero bias in the rest state; driven conduction is outside the static model.
