...
A DAC converts a non-negative integer code $c$ to an analog drive voltage $V_{\mathrm{out}}$. The code domain is unsigned: valid codes run from $0$ to $c_{\max}$ inclusive — the maximum valid code each member reports — and the conversion is defined only on that range. The realized output is the member's nominal code-to-voltage transfer perturbed by its own non-idealities; the family fixes the unsigned code domain and the code-to-voltage direction, leaving the transfer shape and any noise to each member.
...
- **Internals**: [dac base internals](../../../../internals/primitive/analog/dac/base.md)
