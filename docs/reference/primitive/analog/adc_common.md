...
- `AdcOperationPoint` — the frozen `(adc_mode, adc_bits)` pair passed to `convert` per call. `adc_mode` selects a reference tap (valid range `[0, num_refs)`); `adc_bits` is the active resolution, `1 <= adc_bits <= max_bits`.
