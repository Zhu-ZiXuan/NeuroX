...
Omitted effects are finite bandwidth and settling, signal-dependent on-resistance, charge injection, clock feedthrough, off-isolation, crosstalk, DC leakage, and static differential offset. The model is valid where the transported signal lies inside the linear settled band and $g\,\varepsilon_g\,V_{\mathrm{cm}}$ remains below the required resolution.
