...
| `latency_per_op__ns` | combinational window of one add | ns | $\geq 0$ | Design |
...
| $t_{\mathrm{op}}$ | combinational window of one add | ns | `latency_per_op__ns` |
...
- The cost model is behavioural and per-op flat: energy scales only with the output-element count and the duration not at all, neither varying with operand magnitude or carry depth.
