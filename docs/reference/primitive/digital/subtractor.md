...
The subtract is element-wise, so each output element is one subtractor evaluation. The operation runs at once across every fabricated instance, so its duration is the flat combinational window $t = t_{\mathrm{op}}$. Dynamic energy is total work, independent of how the outputs distribute across instances,
