...
The `Driver` is the ideal constant-voltage source-line (SL) clamp driver: it holds the SL boundary of a crossbar column at a fixed drive voltage and absorbs whatever port current the array draws. It is the SL-side boundary actor in the array DC operating-point solve, the counterpart of the BL-side [tia](tia/README.md). It is a leaf circuit a column solver composes directly, not a polymorphic family.
...
$$V_{\mathrm{SL,DR}} = \operatorname{driver}_{\mathrm{SL}}(I_{\mathrm{SL,port}}) = V_{\mathrm{drive}} + n,$$
...
$$\frac{\partial V_{\mathrm{SL,DR}}}{\partial I_{\mathrm{SL,port}}} = 0.$$
...
|---|---|---|---|---|
| drive thermal noise | thermal fluctuation on the buffered drive node | additive zero-mean Gaussian on $V_{\mathrm{SL,DR}}$ | drive-thermal sigma | `drive_thermal` |
...
|---|---|---|---|
| $V_{\mathrm{SL,DR}}$ | SL drive (clamp) voltage | V | `v_sl_drive` |
