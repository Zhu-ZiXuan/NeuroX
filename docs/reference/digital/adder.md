...
The adder is the element-wise integer add primitive of the digital periphery: it computes the sum of two broadcastable integer tensors with no saturation and no wrap. It is an exact digital block; the only modelled physical content is its behavioural PPA cost. Its element-wise twin, subtractor, differs only in sign.

## Physical model

The block abstracts a parallel array of integer adders, one per output element. It applies no fixed-width register semantics: the integer sum is full-precision, so the nominal `bit_width` does not bound the result. The model is purely behavioural — no carry-chain timing, no per-bit gate model — so the only physical quantities it exposes are the PPA cost terms.
...
|---|---|---|---|
| `bit_width` | nominal output bit width; does not bound the result (no wrap) | — | Design |
...
- No saturation and no wrap: the result is the full-precision integer sum, not range-bounded by the nominal `bit_width`.
...
- **Internals**: [adder internals](../../internals/digital/adder.md)
