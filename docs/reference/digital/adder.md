...
The add is element-wise, so each output element is one adder evaluation. The serial-op count of a call is the number of output elements per fabricated instance,
...
Provenance terms: [parameter_provenance](../parameter_provenance.md). File-level schema: `neurox/digital/adder.py` (`AdderConfig`).
