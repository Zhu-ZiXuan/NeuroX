...
- **DCOP (DC operating point)** — the steady-state node-voltage and branch-current solution of the array under the interconnect parasitics and boundary clamp-drivers, found by the [solver](xbar/solver.md) via damped Newton iteration. The `circuit_core` holds no DCOP of its own; the per-call solve state is a transient container.
