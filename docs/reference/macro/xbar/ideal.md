...
N/A — the ideal macro models no hardware. It exercises the same value-domain contract as the physical modes (the accepted weight and activation ranges, the integer matmul protocol) but skips every tiling, organize, aggregate, and analog step. It is the integer reference any physical mode converges to in the no-noise limit.
...
N/A — the ideal macro is lossless by construction. Its ADC surface is the sentinel (`adc_mode_num = 1`, `adc_max_bits = 0`, $s = 1$); the `adc_max_bits = 0` sentinel signals "no output quantization" to the operator.
