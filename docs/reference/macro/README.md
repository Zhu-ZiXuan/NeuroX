...
- [base](base.md) — the abstract macro contract: the integer-matmul protocol every family member satisfies, the two axes (precision slicing $S_w$/$S_a$, matrix tiling $T_r$/$T_c$) and their decompose $\leftrightarrow$ aggregate dual, and the value-domain / ADC surface it publishes.
