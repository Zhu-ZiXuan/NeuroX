...
def _conv_macro(factory: MacroFactory, out_channels: int, in_channels: int, kernel_size: int) -> LinearUnit:
...
def _linear_macro(factory: MacroFactory, out_features: int, in_features: int) -> LinearUnit:
...
class QuantLeNet5(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = QuantConv2d.from_state(
            ...
            quantization_mode=_LAYER_MODE["conv1"],
        )
        self.relu1 = nn.ReLU()
        self.pool1 = nn.MaxPool2d(2, 2)
        self.conv2 = QuantConv2d.from_state(
            ...
            quantization_mode=_LAYER_MODE["conv2"],
        )
        self.relu2 = nn.ReLU()
        self.pool2 = nn.MaxPool2d(2, 2)
        self.flatten = nn.Flatten()
