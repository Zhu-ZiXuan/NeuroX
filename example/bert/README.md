...
- **Output quantization**: the ideal config declares the maximum converter resolution (`adc_bits`) plus one canonical inclusive window `[lower, upper]`, in MAC units, per quantization mode. A call supplies `quantization_mode` and `adc_active_bits`; passing `None` for the latter requests the ideal macro's exact result, and the ideal macro is the output-code rescale reference.
